...
| **トラフィック制御** | 無制限 | AI Gateway レート制限 (QPM=3) → 超過は HTTP 429 |
...
### 画面（6 タブ）
1. **概要**（最左・初期表示）— 構成図・比較表・実際のエンドポイント構成（ライブ取得）。
2. **チャット** — トグルで挙動比較。応答にガードレール判定バッジ（ブロックしたポリシー名・検知理由・監査記録）を表示。
3. **監査ログ** — ガバナンスあり時のリクエストを一覧表示。なし時は「記録なし」。
4. **トラフィック制御** — 10 回連続呼び出しを実行し、各リクエスト/応答をライブ表示。QPM=3 超過分は 429。
5. **入出力管理** — 同じ入力を両モードへ同時送信し、ブロックの違いを左右比較。
6. **使用状況**（最右）— `system.ai_gateway.usage` を集計し、消費トークン(in/out/total)・使用モデル・
   リクエスト元(ユーザー/SP)・エンドポイント別・推定コスト・最近のリクエストを表示。

> 監査ログ・入出力管理・チャットの **監査記録(request_id)** は、ワークスペースの監査ログ
> （`system.ai_gateway.usage` の Catalog Explorer ページ）へのハイパーリンクになっています。
> コストはトークン数 × 概算単価による推定です（環境変数 `USAGE_COST_PER_1K_USD`、既定 0.002）。
...
1. **概要タブ**で構成と 3 観点を説明。
2. **チャットタブ**（トグル OFF＝ガバナンスなし）で「PII: カード番号照会」を送信
   → 機微情報を含む要求がそのまま処理され、監査記録も残らないことを見せる。
3. トグルを **ON**（ガバナンスあり）にして同じ・または「不適切（危険物）」「ジェイルブレイク」を送信
   → ネイティブ service policy（`gurdrail_custom_PII` / `gurdrail_unsafe_content` /
   `gurdrail_jail_break`）が**ブロック**し、ポリシー名・検知理由・監査記録 ID が表示されることを見せる。
   ※ PII は LLM 判定のため文脈依存。確実に見せるなら明確に機微な「カード番号照会」を使う。
5. **監査ログタブ**で、ガバナンスありのリクエストのみが記録されている（なしは 0 件）ことを見せる。
6. **トラフィック制御タブ**で 10 回連続呼び出し → 各リクエスト/応答がライブ表示され、ON では QPM=3 超過分(7件)が 429、OFF では全通過。
